...
def create_interactive_html(fig_list, alignment_file_name, output_file, include_plotly_js):
    ...
    for idx, (_, mapping_method, fig) in enumerate(fig_list):
        ...
        if include_plotly_js == "integrated":
            if idx == 0:
                ...
                html_string += fig.to_html(config={"toImageButtonOptions": {"format" : "svg"}}, full_html=False, default_height="600px", default_width="80%")
                html_string += "</div>\n"
            else:
                html_string += "<div class=plot>\n"
                html_string += fig.to_html(config={"toImageButtonOptions": {"format" : "svg"}}, full_html=False, include_plotlyjs=False, default_height="600px", default_width="80%")
                html_string += "</div>\n"
        elif include_plotly_js == "online":
            if idx == 0:
                html_string += "<div class=plot>\n"
                html_string += fig.to_html(config={"toImageButtonOptions": {"format" : "svg"}}, full_html=False, include_plotlyjs="cdn", default_height="600px", default_width="80%")
                html_string += "</div>\n"
            else:
                html_string += "<div class=plot>\n"
                html_string += fig.to_html(config={"toImageButtonOptions": {"format" : "svg"}}, full_html=False, include_plotlyjs=False, default_height="600px", default_width="80%")
            ...
            if idx == 0:
                html_string += "<div class=plot>\n"
                html_string += fig.to_html(config={"toImageButtonOptions": {"format" : "svg"}}, full_html=False, include_plotlyjs="directory", default_height="600px", default_width="80%")
                html_string += "</div>\n"
            else:
                html_string += "<div class=plot>\n"
                html_string += fig.to_html(config={"toImageButtonOptions": {"format" : "svg"}}, full_html=False, include_plotlyjs=False, default_height="600px", default_width="80%")
...
def parse_bam_file_names(bam_folder):
    """Parse the bam files names."""

    bam_dict = {}
    for f in bam_folder.glob("*.bam"):
...
def parse_genome_lengths(genome_file_path):
    ...
    genome_length_dict = {}
    with open(genome_file_path, "r", encoding="utf-8") as genome_file:
...
def save_dataframes_to_excel(df, output_path):
    ...
    # Use the excel_writer function
    excel_writer(output_path, df_dict)
...
def excel_writer(output_path, data_frames):
    ...
    for sheetname, df in data_frames.items():
        ...
        sheet_name_truncated = sheetname[:31]
